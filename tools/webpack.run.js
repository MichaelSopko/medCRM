      ...
      compiler.plugin('compilation', (compilation) => {
        compilation.plugin('after-optimize-assets', (assets) => {
          ...
          if (mapKey) {
            ...
            for (const idx in srcMap.sources) {
  ...
  compiler.plugin('done', (stats) => {
    ...
    if (stats.compilation.assets['assets.json']) {
      ...
      if (pkg.app.webpackDll) {
        const json = JSON.parse(fs.readFileSync(path.join(pkg.app.frontendBuildDir, 'vendor_dll_hashes.json')));
...
function isDllValid() {
  try {
    ...
    const meta = JSON.parse(fs.readFileSync(hashesPath));
    ...
    const json = JSON.parse(fs.readFileSync(path.join(pkg.app.frontendBuildDir, 'vendor_dll.json')));
